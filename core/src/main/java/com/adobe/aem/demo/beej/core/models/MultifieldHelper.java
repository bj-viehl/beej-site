...
public final class MultifieldHelper {
    ...
    public void activate() {
        ...
        } else if (StringUtils.isNotBlank(multifieldNodeName)) {
    ...
    public String getItemsListAsString() {
        ...
        while (nodesItemList.hasNext()) {
            ValueMap valueMap = nodesItemList.next().getValueMap();
            resourceList.add(valueMap.get(multifieldNodeProperty).toString());
        }
     ...
     *
     * @param page
     *            : page where to look (or start looking) for the nested multifield
     *            node.
    ...
    private Iterator<Resource> getMultifieldItemsResources(Page page) {
        if (page != null) {
            ...
            if (contentResource != null && contentResource.getChild(multifieldNodeName) != null) {
                return getItemsFromResource(contentResource);
            }
            if (inheritProperty) {
    ...
    private Iterator<Resource> getItemsFromResource(Resource componentResource) {
        ...
        java.util.Optional<Resource> multifieldChild = java.util.Optional
                .ofNullable(componentResource.getChild(multifieldNodeName));
        if (multifieldChild.isPresent()) {
            return multifieldChild.get().listChildren();
        }
        return Collections.emptyIterator();
    }
    ...
    private Iterator<Resource> sortListByKey(Resource componentResource) {
        ...
        if (multifieldChild.isPresent()) {
            languageIterator = multifieldChild.get().listChildren();
        }
